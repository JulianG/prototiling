 ...
 */
define(['lib/signals','lib/easeljs/DisplayObjectPool'], function (signals,DisplayObjectPool) {
	...
	api.getDisplayObject = function getDisplayObject(name){
		...
		if(pool){
			...
			throw('[EaselJSAtlas] Error. Canno find an object pool named: ' + name);
		}
		return obj;
	};
	...
	api._initBitmap = function _initBitmap(ss, name) {
		var img = createjs.SpriteSheetUtils.extractFrame(ss, name);
		var bmp = new createjs.Bitmap(img);
		var pool = new DisplayObjectPool( bmp );
		//pool.init(100);
	...
	api._initBitmapAnimation = function _initBitmapAnimation(ss, name) {
		...
		var pool = new DisplayObjectPool( bmpa );
		//pool.init(100);
		this.objectPools[name] = pool;
	};
